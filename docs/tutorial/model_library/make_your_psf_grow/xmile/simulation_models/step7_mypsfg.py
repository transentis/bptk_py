...
class simulation_model():
    def __init__(self):
        ...
        'professionalStaff'          : lambda t: ( (200.0) if ( t  <=  self.starttime ) else (self.memoize('professionalStaff',t-self.dt) + self.dt * ( self.memoize('staffArriving',t-self.dt) )) ),
        'projects'          : lambda t: ( (320.0) if ( t  <=  self.starttime ) else (self.memoize('projects',t-self.dt) + self.dt * ( self.memoize('winningProjects',t-self.dt) - ( self.memoize('deliveringProjects',t-self.dt) ) )) ),
        'proposals'          : lambda t: ( (320.0) if ( t  <=  self.starttime ) else (self.memoize('proposals',t-self.dt) + self.dt * ( self.memoize('prospectingProjects',t-self.dt) - ( self.memoize('winningProjects',t-self.dt) ) )) ),
        'receivables'          : lambda t: ( (17.6 * 160.0 * 2.0) if ( t  <=  self.starttime ) else (self.memoize('receivables',t-self.dt) + self.dt * ( self.memoize('makingRevenue',t-self.dt) - ( self.memoize('collectingRevenue',t-self.dt) ) )) ),
        'staffInRecruitment'          : lambda t: ( (0.0) if ( t  <=  self.starttime ) else (self.memoize('staffInRecruitment',t-self.dt) + self.dt * ( self.memoize('hiringStaff',t-self.dt) - ( self.memoize('staffArriving',t-self.dt) ) )) ),
        
    
        # Flows
        'cashIn'             : lambda t: max([0 , self.memoize('collectingRevenue', t)]),
        'cashOut'             : lambda t: max([0 , self.memoize('cost', t)]),
        'collectingRevenue'             : lambda t: max([0 , self.delay( self.memoize('makingRevenue', ( t - (self.memoize('collectionTime', t)) )),self.memoize('collectionTime', t),17.6 * 160.0,t)]),
        'deliveringProjects'             : lambda t: max([0 , self.memoize('projectDeliveryRate', t)]),
        'hiringStaff'             : lambda t: max([0 , self.memoize('hiringRate', t)]),
        'makingRevenue'             : lambda t: max([0 , self.memoize('revenue', t)]),
        'prospectingProjects'             : lambda t: max([0 , self.memoize('projectProposalRate', t)]),
        'staffArriving'             : lambda t: max([0 , self.delay( self.memoize('hiringStaff', ( t - (self.memoize('hiringDuration', t)) )),self.memoize('hiringDuration', t),0.0,t)]),
        'winningProjects'             : lambda t: max([0 , self.delay( self.memoize('prospectingProjects', ( t - (self.memoize('projectAcquisitionDuration', t)) )),self.memoize('projectAcquisitionDuration', t),160.0,t)]),
        
    
        # converters
        'businessDevelopmentCapacity'      : lambda t: self.memoize('workCapacity', t) * self.memoize('businessDevelopmentAllocation%', t) / 100.0,
        ...
        'hiringDuration'      : lambda t: 3.0,
        'overheadCost'      : lambda t: 306.0,
        'projectAcquisitionDuration'      : lambda t: 6.0,
        'projectDeliveryCapacity'      : lambda t: self.memoize('workCapacity', t) - self.memoize('businessDevelopmentCapacity', t),
        'projectDeliveryFee'      : lambda t: 17.6,
        'projectDeliveryRate'      : lambda t: min([self.memoize('projects', t) , self.memoize('projectDeliveryCapacity', t)]),
        'projectProposalRate'      : lambda t: self.memoize('projectVolume', t) * ( self.memoize('businessDevelopmentCapacity', t) / self.memoize('prospectingEffort', t) ),
        'projectVolume'      : lambda t: 16.0,
        'prospectingEffort'      : lambda t: 4.0,
        'revenue'      : lambda t: self.memoize('projectDeliveryFee', t) * self.memoize('projectDeliveryRate', t),
        'staffCost'      : lambda t: ( self.memoize('staffSalary', t) + self.memoize('workplaceCost', t) ) * self.memoize('professionalStaff', t),
        'staffSalary'      : lambda t: 80.0 / 12.0,
        'workCapacity'      : lambda t: self.memoize('workMonth', t) * self.memoize('professionalStaff', t),
        'workMonth'      : lambda t: 1.0,
        'workplaceCost'      : lambda t: 1.0,
        
    
        # gf
        'businessDevelopmentAllocation%' : lambda t: LERP(  t , self.points['businessDevelopmentAllocation%']),
        'hiringRate' : lambda t: LERP(  t , self.points['hiringRate']),
        ...
        self.points = {
            'businessDevelopmentAllocation%' :  [(0.0, 0.0), (1.0, 0.0), (2.0, 0.0), (3.0, 0.0), (4.0, 0.0), (5.0, 0.0), (6.0, 0.0), (7.0, 0.0), (8.0, 0.0), (9.0, 0.0), (10.0, 0.0), (11.0, 0.0), (12.0, 0.0), (13.0, 0.0), (14.0, 0.0), (15.0, 0.0), (16.0, 0.0), (17.0, 0.0), (18.0, 0.0), (19.0, 0.0), (20.0, 0.0), (21.0, 0.0), (22.0, 0.0), (23.0, 0.0), (24.0, 0.0)]  , 'hiringRate' :  [(0.0, 0.0), (1.0, 0.0), (2.0, 0.0), (3.0, 0.0), (4.0, 0.0), (5.0, 0.0), (6.0, 0.0), (7.0, 0.0), (8.0, 0.0), (9.0, 0.0), (10.0, 0.0), (11.0, 0.0), (12.0, 0.0), (13.0, 0.0), (14.0, 0.0), (15.0, 0.0), (16.0, 0.0), (17.0, 0.0), (18.0, 0.0), (19.0, 0.0), (20.0, 0.0), (21.0, 0.0), (22.0, 0.0), (23.0, 0.0), (24.0, 0.0)]  , 
        }
        ...
        self.stocks = ['cash',   'professionalStaff',   'projects',   'proposals',   'receivables',   'staffInRecruitment'  ]
        self.flows = ['cashIn',   'cashOut',   'collectingRevenue',   'deliveringProjects',   'hiringStaff',   'makingRevenue',   'prospectingProjects',   'staffArriving',   'winningProjects'  ]
        self.converters = ['businessDevelopmentCapacity',   'cashFlow',   'collectionTime',   'cost',   'hiringDuration',   'overheadCost',   'projectAcquisitionDuration',   'projectDeliveryCapacity',   'projectDeliveryFee',   'projectDeliveryRate',   'projectProposalRate',   'projectVolume',   'prospectingEffort',   'revenue',   'staffCost',   'staffSalary',   'workCapacity',   'workMonth',   'workplaceCost'  ]
        self.gf = ['businessDevelopmentAllocation%',   'hiringRate'  ]
        self.constants= []
        self.events = [
            ]
