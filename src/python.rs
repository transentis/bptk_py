//! The Python extension module: everything that knows pyo3 lives here.
//!
...
#[pymethods]
impl RustSdModel {
    /// Register the Python function to call for `name`.
    ///
    /// The callable is called as `fn(t, arg1, ..., argN)`. Anything it needs besides
    /// that - the model it belongs to, say - is bound on the Python side before it gets
    /// here, so the engine holds a callable and knows nothing about what it closes over.
    fn register_function(&mut self, name: &str, function: Py<PyAny>) -> PyResult<()> {
        let slot = self
            .model
            .callback_names
            .iter()
            ...
            .ok_or_else(|| {
                PyValueError::new_err(format!(
                    "This model does not call a function named '{}'. It calls: {}",
                    name,
                    describe_names(&self.model.callback_names)
                ))
            })?;
        self.model.callbacks[slot] = Some(function);
        Ok(())
    }

    /// The names this model calls that already have a callable.
    fn registered_functions(&self) -> Vec<String> {
        self.model
            .callback_names
            .iter()
            .zip(self.model.callbacks.iter())
            .filter(|(_, callable)| callable.is_some())
            .map(|(name, _)| name.clone())
            .collect()
    }

    /// The names this model calls that have no callable yet.
    fn required_functions(&self) -> Vec<String> {
        self.model
            .callback_names
            .iter()
            .zip(self.model.callbacks.iter())
            .filter(|(_, callable)| callable.is_none())
            .map(|(name, _)| name.clone())
            .collect()
    }
...
impl RustSdModel {
    ...
    /// timestep that says nothing about what is missing.
    fn check_functions_registered(&self) -> PyResult<()> {
        let missing = self.required_functions();
        if missing.is_empty() {
            return Ok(());
        }
        Err(PyValueError::new_err(format!(
            "This model calls Python functions that are not registered: {}. \
             Register them with register_function() before running.",
...
/// Names as a readable list, for a message that has to say which ones.
