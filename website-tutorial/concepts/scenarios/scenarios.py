...
def _(mo):
    ...
    The ``model`` property names the (python) simulation model relative to the project directory, which is the parent of the ``scenarios`` folder - either as a path to a module, ``"simulation_models/my_model"``, or as a dotted class, ``"src.my_model.MyModel"``. The working directory does not matter.
    The ``source`` property contains the path, relative to the same project directory, to the original model created in a 3rd party application. The framework provides automatic conversion from .itmx/.stmx (Stella Architect) files. When working with a supported 3rd party application, it is recommended to set the ``source`` property. When a value for ``source`` is provided, a file monitor will run in the background and check for changes in the source model. When changes are found the framework automatically updates the python model.
